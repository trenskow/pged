...
export default class QueryBuilder extends CustomPromise {
	...
	_addParameter(value) {
		...
		const existingValue = this._queryParameters.findIndex((current) => current === value);

		if (existingValue !== -1) return `$${existingValue + 1}`;
		...
		const result = conditions.map((condition) => {
			...
			if (key.substring(0, 1) == ':') {
				...
				if (typeof value === 'string' && value.substring(0, 1) == ':') {
					...
					value = `${this._addParameter(this._formatParameter(key, condition[key]))}`;
				}
			...
			if (!Array.isArray(condition[key])) {
				return this._buildCondition(dbKey, comparer, `${this._addParameter(this._formatParameter(key, condition[key]))}`);
			} else {
				const values = condition[key].map((value) => {
					return `${this._addParameter(this._formatParameter(key, value))}`;
				});
	...
	_buildUpdateKeysAndValues(keys, values) {
		return `set ${keys.map((key, idx) => {
			...
			if (value == null) {
				...
				value = `${this._addParameter(this._formatParameter(key, value))}`;
			}
	...
	_buildInsertValues() {
		return this._insertValues.map((value, idx) => {
			return `${this._addParameter(this._formatParameter(this._insertKeys[idx], value))}`;
		}).join(', ');
	}
